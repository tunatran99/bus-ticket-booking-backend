...
export class DatabaseInitService implements OnModuleInit {
  ...
  private async seedRolesAndPermissions() {
    ...
    for (const name of roleNames) {
      ...
      if (!exists) {
        await this.roles.save(
          this.roles.create({ name, description: `${name} role` }),
        );
    ...
    if (adminRole) {
      const missing = allPerms.filter(
        (p) => !adminRole.permissions?.some((ep) => ep.id === p.id),
      );
      if (missing.length) {
        adminRole.permissions = [...(adminRole.permissions || []), ...missing];
        await this.roles.save(adminRole);
        this.logger.log(
          `Linked ${missing.length} new permissions to admin role.`,
        );
      }
    }
  }
  ...
  private async seedAdminUser() {
    ...
    let admin = await this.users.findOne({
